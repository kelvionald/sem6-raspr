...
namespace Library
{
    public class RedisStorage : IStorage
    {
        ...
        public RedisStorage() {
            ...
            connections.Add(Const.SEGMENT_RUS, ConnectionMultiplexer.Connect(
                Environment.GetEnvironmentVariable("DB_RUS", EnvironmentVariableTarget.User)
            ).GetDatabase());
            connections.Add(Const.SEGMENT_EU, ConnectionMultiplexer.Connect(
                Environment.GetEnvironmentVariable("DB_EU", EnvironmentVariableTarget.User)
            ).GetDatabase());
            connections.Add(Const.SEGMENT_OTHER, ConnectionMultiplexer.Connect(
